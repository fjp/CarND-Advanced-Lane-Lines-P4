...
def processImage(img):
    ...
    innerLane = np.array(list(zip(np.concatenate((leftFitx + windowWidth/2, rightFitx[::-1] - windowWidth/2), axis=0), np.concatenate((yvals, yvals[::-1]), axis=0))), np.int32)
    ...
    result = cv2.addWeighted(base, 1.0, roadWarped, 0.7, 0.0)
...
OutputVideo = 'output1_tracked.mp4'

clip1 = VideoFileClip(InputVideo)
videoClip = clip1.fl_image(processImage) # NOTE: this function expects color images!!
videoClip.write_videofile(OutputVideo, audio=False)
